...
module.exports = function Router(container, route, options, handler) {
  ...
  function subscribe(ch) {
    ...
    function handleMessage(msg) {
      ...
      function ackMessage() {
        debug('ack', 'message', self.options.autoAck);
        ch.ack(msg);
      }

      var middleware;

      if (Array.isArray(self.options.middleware)) {
        middleware = self.container.middleware.concat(self.options.middleware);
      } else {
        middleware = self.container.middleware;
      }
...
//        debug('updateMsg', updateMsg);
        ...
        if(self.options.autoAck) ackMessage();
      }, onErr);
