...
module.exports = function Router(container, route, options, handler) {
  ...
  this.init = function init() {
    ...
    if (self.options.hasOwnProperty('amqpConnection')) {
      // connection provided externally
      debug('init', '<existing connection>', self.route);
      open = when(self.options.amqpConnection);
    } else {
      // no connection specify, connect manually
      debug('init', self.options.amqpUrl, self.route);
      self.consumerTag = consumerTag();

      open = amqp.connect(self.options.amqpUrl);
    }
  ...
  this.cleanup = function(err) {
    self.container.logger.error('Exiting due to connection error: ', err);
    process.exit(-1);
  };
